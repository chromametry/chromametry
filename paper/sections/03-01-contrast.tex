...
Taking the conservative estimate $\lambda = \max(\lambda_1, \lambda_2)$, the same formula $K_{\text{ideal}} = \lceil \lambda \cdot (N-1) \rceil$ applies. Table~\ref{tab:lambda-by-cr} summarizes the derived $\lambda$ values for the four WCAG standard thresholds.
