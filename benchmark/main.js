...
const STATISTICS = {
    contrastEfficiency: Chromametry.calcStatistics(DATA.map(item => item.metrics.contrastEfficiency)),
    lightnessLinearity: Chromametry.calcStatistics(DATA.map(item => item.metrics.lightnessLinearity)),
    chromaSmoothness: Chromametry.calcStatistics(DATA.map(item => item.metrics.chromaSmoothness)),
    hueStability: Chromametry.calcStatistics(DATA.map(item => item.metrics.hueStability)),
    spacingUniformity: Chromametry.calcStatistics(DATA.map(item => item.metrics.spacingUniformity)),
    score: Chromametry.calcStatistics(DATA.map(item => item.metrics.score)),
}
...
const RANKINDDATA = DATA.map((item, i) => {
...
const description = () => {
    ...
    <div class="header-description">
        <div>
This benchmark evaluates <b>sequential color ramps</b> under WCAG 4.5:1.
    ...
    DATA.forEach(org => {
        h += `
            <tr class="palette-row">
                ...
                <td class="val">${org.ramps.length}</td>
                <td class="val">${org.steps}</td>
                <td class="val">${org.wcag[45].span}</td>

                <td class="val heat" style="--heat:${heatUp(org.metrics.contrastEfficiency, STATISTICS.contrastEfficiency)}">
                    ${org.metrics.contrastEfficiency.toFixed(3)}
                </td>

                <td class="val heat" style="--heat:${heatUp(org.metrics.lightnessLinearity, STATISTICS.lightnessLinearity)}">
                    ${org.metrics.lightnessLinearity.toFixed(4)}
                </td>

                <td class="val heat" style="--heat:${heatUp(org.metrics.chromaSmoothness, STATISTICS.chromaSmoothness)}">
                    ${org.metrics.chromaSmoothness.toFixed(4)}
                </td>

                <td class="val heat" style="--heat:${heatUp(org.metrics.hueStability, STATISTICS.hueStability)}">
                    ${org.metrics.hueStability.toFixed(4)}
                </td>

                <td class="val heat" style="--heat:${heatUp(org.metrics.spacingUniformity, STATISTICS.spacingUniformity)}">
                    ${Number(org.metrics.spacingUniformity).toFixed(4)}
                </td>

                <td class="val heat" style="--heat:${heatUp(org.score, STATISTICS.score)}">
...
const backgroundGrid = (item) => {
    ...
    let h = `
        <div class="table-section">
          <div class="table-caption">Contrast Map: Background Index ± ${item.wcag[45].span} steps (WCAG 4.5:1). "X" indicates invalid background </div>
...
const metricTable = (scales) => {
    ...
    scales.forEach(s => {
        h += `
            <div class="grid-cell">
                <div style="background:${s.colors[s.baseIndex]};
                            width:40px;height:20px;border-radius:2px"></div>
            </div>

            <div class="grid-cell">${s.wcag[30]?.span ?? '-'}</div>
            <div class="grid-cell"><strong>${s.wcag[45]?.span ?? '-'}</strong></div>
            <div class="grid-cell">${s.wcag[70]?.span ?? '-'}</div>
...
const detail = () => {
    ...
    DATA.forEach(item => {
        ...
        detailEl.innerHTML = `<div class="details-head">${item.name}</div><div class="details-body">${backgroundGrid(item)}${metricTable(item.ramps)}</div>`;
        ...
        ["lightness", "chroma", "hue", "cumDeltaE00"].forEach(e => {
            ...
            new Chart(canvas, {
                type: "line",
                data: {
                    labels: item.ramps[0].colors.map((_, i) => i),
                    datasets: item.ramps.map(s => ({
                        label: s.name,
                        data: e == "hue"
                            ? s.chart[e].map((m, i) => ({ x: i + 1, y: m}))
                            : s.chart[e].map((m, i) => ({ x: i, y: m})),
                        borderColor: s.colors[s.baseIndex],
                        borderWidth: 1,
                        fill: false,
                        pointRadius: 0
                    }))
