import { MonochromaticPaletteData, rgbToHex } from "../../../src/index.js";
...
const palette: Record<string, Record<number | string, string>> = { azure, blue, gray, green, lime, magenta, orange, purple, red, rose, teal, yellow };
const colors: Record<string, string[]> = {}
for (let name in palette) {
    const rgbs = palette[name] as Record<number | string, string>
    colors[name] = Object.values(rgbs).map(str => {
        let [r, g, b] = str.match(/\d+(\.\d+)?/g)!.map(e => Number(e) / 255)
        return rgbToHex([r, g, b])
    })
    colors[name][0]="#ffffff"
    colors[name].push("#000000");
}
const stepNames = [...Array(17).keys()]
const shopify: MonochromaticPaletteData = {
